...
char nlet(char *ptr){
  // If the ptr is not pointing towards a character in the array
  // Checks if the ptr is pointing to 'Z';
...
int ldif(char c1, char c2){
  // Check if the inputted chars are valid capital letters
  if (c1 < 'A' || c1 > 'Z' || c2 < 'A' || c2 > 'Z' ) {
    return -100;
  }
  return pmatch(c2) - pmatch(c1);
}
...
char* per_print(Person *p, char* personbuf){
  ...
  sprintf(personbuf + strlen(personbuf), " First Name:       %.20s\n", p->FirstName);
  sprintf(personbuf + strlen(personbuf), " Last Name:        %.30s\n", p->LastName);
  sprintf(personbuf + strlen(personbuf), " Address:          %.60s\n", p->StreetAddr);
  sprintf(personbuf + strlen(personbuf), " Zip:              %.6s\n", p->ZipCode);

  // Convert double value to int for printing
  float_to_ints((float) round(p->Height * 100) / 100, output);
  sprintf(personbuf + strlen(personbuf), " Height:           %d.%d\n", output[0], output[1]);

  // Convert float value to int for printing
  float_to_ints((float) round(p->Weight * 100) / 100, output);
  sprintf(personbuf + strlen(personbuf), " Weight:           %d.%d\n", output[0], output[1]);
